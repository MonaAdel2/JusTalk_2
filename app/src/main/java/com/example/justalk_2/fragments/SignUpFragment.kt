...
import com.example.justalk_2.mvvm.AuthViewModel
...
//            activity?.supportFragmentManager?.popBackStack()
    ...
    private fun signUpWithEmailAndPassword(username: String, email: String, password: String, imageBitmap: Bitmap?) {
...
//            bitmap = imageBitmap
    ...
    private val requestImagePick = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == Activity.RESULT_OK) {
...
//            uploadImageToFirebaseStorage(imageBitmap)
    ...
    private fun uploadImageToFirebaseStorage(imageBitmap: Bitmap?, onSuccess: (imageUrl: String) -> Unit) {
        if (imageBitmap != null) {
